...
- 此边缘检测仅利用了颜色信息，在实际应用中，物体的纹理、阴影等信息均会影响边缘检测的记过，使结果包含了许多非预期的描边，为得到更准确的边缘信息，往往会在屏幕的深度纹理和发现纹理上进行边缘检测
...
- 运动模糊又很多中实现方法：
  ...
  2. 应用广泛的一种方法，创建和使用速度魂村，该缓存中存储了各个像素当前的运动速度，然后利用该值来决定模糊的方向和大小
